...
Ubuntu:
```bash
blender ./blender-projects/Zamuza.blend --python ./scripts/examples/retarget_zamuza_to_g1.py
uv run ./scripts/compute_dof_ik.py --motion ./data/motions/g1_zamuza_0_1632_keypoint.npz --mapping MMD_YYB_TO_G1_MAPPING
```
...
motion = build_body_motion_data(source_armature, config=MMD_YYB_TO_G1_CFG)

# blender is +Y forward, we need to rotate to +X forward
motion = rotate_motion(motion, np.pi / 2)

save_path = f"./data/motions/g1_zamuza_{motion_section[0]}_{motion_section[1]}_keypoint.npz"
